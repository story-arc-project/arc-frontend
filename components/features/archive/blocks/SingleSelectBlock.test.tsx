 ...
 */
function Harness({ initial = OPTIONS, selected = "" }: { initial?: string[]; selected?: string }) {
...
describe("SingleSelectBlock 옵션 편집", () => {
  ...
  it("지운 옵션이 선택돼 있었으면 선택을 비운다", async () => {
    const user = userEvent.setup()
    render(<Harness initial={["정규직", "계약직"]} selected="정규직" />)
    await openEditor(user)
  ...
  it("마지막 옵션이 남았을 때 삭제를 눌러도 onChange 가 불리지 않는다", async () => {
    ...
    render(<SingleSelectBlock block={makeBlock(["정규직"])} onChange={onChange} />)
    await openEditor(user)
