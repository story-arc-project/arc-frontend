...
ARC는 **모바일 우선(Mobile First)** 원칙을 따른다. 기본 스타일은 모바일 기준으로 작성하고, `sm:` 이상의 브레이크포인트로 데스크톱을 확장한다.
...
버튼 안 텍스트가 좁은 화면에서 두 줄로 깨질 경우, 레이아웃을 먼저 조정한다 (`grid-cols-1` 전환 등). `whitespace-nowrap`은 레이아웃 조정이 불가능할 때만 사용한다.
