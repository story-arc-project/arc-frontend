...
지점은 이 한 곳뿐이다. 값을 읽을 수 없는 블록(`isUnrenderableBlock`)에서는 바깥 연필과
같은 술어로 함께 닫힌다 — 두 편집 경로가 한쪽만 열린 채 남지 않는다.

**예외 하나 — 그릴 항목이 0개인 체크리스트는 잠기지 않는다.** `BlockRenderer`는 mood-tag가
그릴 태그를 하나도 못 구했을 때만 `ChecklistBlock`으로 폴백한다(FRT-177). 그 자리에서 추가까지
막으면 값을 넣을 방법이 **0인 죽은 필드**가 된다 — 값을 살리려 만든 폴백이 값을 막는 통로가
된다. `ChecklistBlock`이 스스로 하한선을 지킨다: `allowOptionEdit || options.length === 0`.

### 2) 저장된 값은 숨지 않는다 — 목록의 소유권을 편집 권한과 같은 축으로 가른다
...
처방은 합집합이 **아니다** — 합집합으로 만들면 커스텀 블록에서 사용자가 방금 지운 옵션이
`block.options`(생성 시점 프리셋)에서 매 렌더 부활한다. FRT-158이 정확히 그 버그였다.
소유권을 같은 플래그로 가른다:

| | 목록을 소유하는 쪽 | 계산 |
|---|---|---|
| `allowOptionEdit` (커스텀) | 사용자 | `saved.length > 0 ? saved : preset` (현행 유지) |
| 미지정 (템플릿) | 확정본 | `preset.length > 0 ? preset : saved` |

어느 쪽이든 저장된 **값**(`selected`)이 목록에 없으면 뒤에 붙여 보존한다.
`MoodTagBlock.moodTagOptions()`의 "checked에만 남은 값도 뒤에 붙인다"와 같은 규약이다.
선택되지 않은 커스텀 옵션이 템플릿 블록에서 사라지는 것은 값 손실이 아니다 — 값은 `selected`
하나뿐이다.

⚠️ 이 보정은 **표시 전용**이다. `onChange`가 나갈 때 목록을 굳혀 쓰지 않는다 — 굳히면 다음
템플릿 개편이 그 필드에 닿지 못한다.
...
- 텍스트칸을 비우면 `selected`는 `""`(미선택)이 된다. **필수 필드는 `required`를 입력칸으로
  넘긴다** — '기타' 모드에서 select의 값은 `"기타"`라, 거기에 `required`를 두면 저장값이 비어
  있는데도 브라우저 검증이 통과한다.
