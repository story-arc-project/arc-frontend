...
const mockUsePathname = vi.fn(() => "/analysis/keyword");
...
beforeEach(() => {
  ...
  mockUsePathname.mockReturnValue("/analysis/keyword");
});
...
// 데모는 둘러보기만 한다 — 만들기·지우기는 mock 위에서 화면만 바꾸고 재조회하면 되살아난다(FRT-232).
describe("데모 모드 — 둘러보기만 남긴다 (FRT-232)", () => {
  it("'새 키워드 분석' 버튼이 헤더에서 사라진다", async () => {
    mockUsePathname.mockReturnValue("/demo/analysis/keyword");
    getList.mockResolvedValue([snap("k1", "completed")]);

    render(<KeywordAnalysisPage />);
    await flush();

    expect(screen.queryByRole("link", { name: "새 키워드 분석" })).toBeNull();
  });
  ...
  it("삭제·즐겨찾기 버튼이 없고, 상세 링크는 데모 안에 머문다", async () => {
    ...
    expect(screen.getByRole("link", { name: /분석 k1/ })).toHaveAttribute(
      "href",
      "/demo/analysis/keyword/k1",
    );
  });
  ...
  it("일반 모드는 그대로다 (거울상 — 데모 분기가 본계약을 먹지 않았는지)", async () => {
    ...
    expect(screen.getByRole("link", { name: "새 키워드 분석" })).toBeInTheDocument();
