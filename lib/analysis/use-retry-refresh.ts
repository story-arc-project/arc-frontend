...
export function useRetryRefresh(
  ...
  useEffect(() => {
    ...
    //
    // ⚠️ 이 비겹침은 **한 라운드 안에서만** 성립한다. 폴링 중에 다른 카드를 재시도하면
    // round 가 올라가며 이 정리 함수가 타이머만 지우고, 이미 나가 있는 요청은 남는다
    // (fetch 를 취소하지 않는다) — 새 라운드의 요청과 잠깐 겹칠 수 있다.
    // 그래도 상태는 안전하다: 새 라운드는 반드시 onRetried 를 거치고 거기서 호출부의
    // mutationEpoch 가 올라가므로, 남은 옛 요청의 응답은 반드시 폐기된다. 남는 비용은
    // 드물게 나가는 GET 한 번이라, 라운드 간 취소 장치를 더 얹지 않았다.
    const schedule = () => {
