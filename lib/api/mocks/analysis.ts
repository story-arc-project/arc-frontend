...
export const mockIndividualAnalysisResult: IndividualAnalysisResult = {
  ...
  result: {
    ...
    deepAnalysis: {
      careerValue:
        "데이터 품질 관리와 실험 재현성은 모델링 역량보다 검증하기 어려운 축입니다. 두 가지를 수치로 증명한 이력은 ML 엔지니어·데이터 사이언티스트 지원 시 곧바로 신뢰 근거가 됩니다. 특히 6개월이라는 짧은 기간에 가이드라인 개정과 파이프라인 정비라는 성격이 다른 두 결과물을 남겼고, 둘 다 '남이 다시 확인할 수 있는 형태'로 정리했다는 점이 이 경험의 핵심 가치입니다. 학부 연구생 단계에서 연구 보조가 아니라 연구 기반을 정비한 사례로 읽힙니다.",
      applicableRoles: [
        "ML 엔지니어",
        "데이터 사이언티스트",
        "NLP 리서치 엔지니어",
      ...
      marketValue:
        "데이터 중심 접근(Data-centric AI)은 최근 채용에서 비중이 커진 축이고, 일치도·재현성처럼 정량화된 근거를 가진 지원자는 많지 않습니다. 대부분의 학부 지원자가 '모델을 무엇으로 바꿨다'를 적을 때 '데이터를 어떻게 믿게 만들었다'를 적을 수 있다는 점에서 서류 단계의 변별력이 큽니다. 다만 이 강점은 연구직에 가까울수록 크게 읽히고, 서비스 분석 직무에서는 비즈니스 지표와의 연결을 함께 보여줘야 같은 무게로 전달됩니다.",
    },
    starFormat: {
      title: "레이블링 기준 개정으로 데이터 일치도 0.61 → 0.78",
      situation:
        "레이블러 4명의 판단이 경계 사례마다 갈려 데이터셋의 신뢰도를 보장할 수 없었고, '인용된 혐오 표현'을 혐오로 볼지에서는 2주간 결론이 나지 않던 상황",
      task:
        "판단이 갈리는 경계 사례를 유형별로 정리해 같은 문장이면 같은 레이블이 붙게 만들고, 개선 여부를 같은 표본으로 다시 확인해 수치로 증명하는 것",
      action:
        "불일치 사례 320건을 6개 패턴으로 분류하고, 패턴마다 판단 기준과 예시 문장을 붙여 가이드라인을 개정했습니다. 의견이 갈린 항목은 두 기준으로 각각 레이블링해 성능 차이를 잰 뒤 그 결과를 근거로 확정했고, 개정본을 같은 표본에 다시 적용해 재측정했습니다.",
      result:
        "레이블러 간 일치도(Cohen's Kappa)가 0.61에서 0.78로 올랐고, 판단이 가장 많이 갈리던 풍자·인용 표현에서 불일치가 가장 크게 줄었습니다.",
    },
    // v1.2 신설. 이 시드는 STAR 4칸이 다 차 있으므로 백엔드라면 null 을 보낸다.
    starNote: "",
    // v1.2 신설. 실재성 검증에서 걸러낸 추천 — 이 시드는 걸러낸 항목이 없다.
    removedRecommendations: [],
    // FRT-271: 백엔드 individual.py 는 강점을 deep_analysis 가 아니라 최상위 item_strengths 에 둔다.
    // ⚠️ 아래 서술은 전부 이 시드 자신의 STAR·진단 문장에서 가져온 것이다 — 데모가 스스로
    // 반박하지 않도록, 진단이 "아직 없다"고 적은 값(개정 전후 모델 성능)은 강점으로 쓰지 않는다.
    itemStrengths: {
      hasGenuineStrengths: true,
      oneLineVerdict:
        "데이터를 믿게 만든 과정이 수치로 남아 있는 것이 이 경험의 핵심입니다.",
      noStrengthReason: "",
      summarizedStrengths: [
        "불일치 사례 320건을 6개 패턴으로 유형화해 판단 기준을 문서로 만든 경험",
        "seed 고정·설정 파일 분리·자동 기록으로 실험 재현성을 확보한 경험",
        "의견이 갈린 지점을 논쟁이 아니라 실험으로 결론 낸 의사결정 방식",
        "재현되지 않는 원인을 로그 대조로 데이터 로더 셔플 시드까지 좁혀 특정한 디버깅 과정",
      ],
      strengths: [
        {
          id: "s-1",
          ...
          title: "판단 기준 문서화",
          analysis:
            "불일치 사례 320건을 6개 패턴으로 나누고 패턴마다 판단 기준과 예시 문장을 붙여, 같은 문장이면 같은 레이블이 붙게 만들었습니다.",
          evidence: "레이블러 간 일치도(Cohen's Kappa) 0.61 → 0.78",
          careerImpact:
            "데이터 품질을 감이 아니라 수치로 관리해 본 사람이라는 근거가 됩니다.",
          leverageAction:
            "개정한 가이드라인의 목차와 패턴 6개를 한 장으로 정리해 포트폴리오 앞에 두세요.",
          showcaseExample:
            "Before: 레이블링 가이드라인을 개선했습니다 → After: 불일치 320건을 6개 패턴으로 정리해 기준을 개정하고, 같은 표본 재측정으로 일치도를 0.61에서 0.78로 올렸습니다",
        },
        {
          id: "s-2",
          category: "차별화_포인트",
          ...
          title: "실험 재현성 확보",
          analysis:
            "seed 고정·설정 파일 분리·실행 기록 자동화로 같은 설정이면 같은 결과가 나오게 만들었습니다.",
          evidence: "반복 실행 시 지표 편차 ±0.004",
          careerImpact:
            "결과를 남이 다시 확인할 수 있게 만드는 역량은 연구·엔지니어링 양쪽에서 드물게 검증됩니다.",
          leverageAction:
            "재현 절차를 README 한 페이지로 정리해 저장소 첫 화면에 두세요.",
          // 백엔드는 예시가 없으면 null 을 보낸다 — 데모에서도 그 경우를 함께 보인다.
          showcaseExample: "",
        },
        {
          id: "s-3",
          ...
          title: "원인 좁히기",
          analysis:
            "재현되지 않는 원인을 로그 대조로 데이터 로더 셔플 시드까지 좁혀 특정했습니다.",
          evidence: "'로그를 대조해 데이터 로더의 셔플 시드까지 원인을 좁혔습니다'",
          careerImpact: "막힌 지점을 감이 아니라 절차로 좁힌다는 신호입니다.",
          leverageAction:
            "어디까지 의심했고 무엇으로 지웠는지를 3단계로 적어 두세요.",
          showcaseExample: "",
        },
      ],
      strongestAsset: "레이블 기준을 다시 세워 일치도를 0.61에서 0.78로 올린 경험",
      positioningTip:
        "'모델을 무엇으로 바꿨나'가 아니라 '데이터를 어떻게 믿게 만들었나'로 이야기를 여세요. 재현성 이야기는 그 뒤에 붙이면 한 흐름이 됩니다.",
    },
    itemDiagnosis: {
      oneLineVerdict:
      ...
      limitations: [
        ...
        "데이터셋 규모(문장 수)와 레이블 분포가 없어 개선 폭의 크기를 가늠할 수 없음",
        "파트타임 근무라 주당 투입 시간이 드러나지 않아 성과 대비 몰입도를 판단하기 어려움",
      ],
      weaknesses: [
        {
          id: "w-1",
        ...
        },
        {
          ...
          category: "규모 명시",
          severity: "minor",
          title: "다룬 데이터의 크기가 드러나지 않음",
          diagnosis:
            "불일치 사례 320건은 적혀 있지만 전체 데이터셋이 몇 건인지가 없어, 320건이 큰 표본인지 일부인지 판단할 수 없습니다.",
          evidence: "'레이블러 4명의 불일치 사례 320건을 유형별로 모아' — 모수에 해당하는 수치 없음",
          impact: "같은 개선 폭이라도 규모에 따라 난이도가 달라 성과가 저평가될 수 있음",
          priorityAction: "전체 문장 수와 레이블 클래스 수를 한 줄로 추가하기",
          improvementExample:
            "'총 1만 2천 문장, 4개 클래스 중 불일치 320건을 대상으로 했습니다.'",
        },
      ],
      missingElements: [
        "개정 전후 모델 성능 비교",
    ...
    // v1.2 — 백엔드가 절대 기간·마감일을 덧씌운 형태(normalize_time_fields).
    actionPlan: {
      shortTerm: {
        period: "2026-09-01 ~ 2026-12-01",
        deadline: "2026-12-01",
        content:
          "개정 전후 데이터로 동일 모델을 학습해 F1 차이를 기록에 추가하세요. 파이프라인이 이미 재현 가능하므로 설정 파일만 바꿔 두 번 돌리면 되고, 전체 문장 수와 클래스 분포도 이때 함께 적어두면 규모 공백이 같이 메워집니다.",
      },
      midTerm: {
        period: "2026-12-01 ~ 2027-09-01",
        deadline: "2027-09-01",
        content:
          "가이드라인 개정 과정을 회고 글로 정리해 공개하세요. 판단 기준을 만드는 과정은 공개 사례가 드물어 그 자체로 차별점이 되고, 본인이 먼저 제안해 채택된 결정(focal loss 적용 등)을 함께 적으면 지금 흐릿한 역할 분담도 또렷해집니다.",
      },
      longTerm: {
        period: "2027-09-01 ~ 2029-09-01",
        deadline: "2029-09-01",
        content:
          "데이터 품질 관점의 기여를 오픈소스 데이터셋 프로젝트로 확장하세요. 교내 연구실 밖에서 같은 방식이 통한다는 것을 보이면, 이 경험이 특정 환경에 한정된 성과가 아니라는 근거가 됩니다.",
      },
    },
    missingInfoWarning:
...
export const mockComprehensiveList: AnalysisSnapshot[] = [
  {
    ...
    status: "completed",
    createdAt: "2026-06-07T12:00:00Z",
    // 분석에 넣은 건 6건인데 그중 1건은 이후 삭제돼 id 로 더 이상 찾을 수 없다.
    // 그래서 count(6) 와 조회 가능한 id 수(5) 가 다르다 — 상세의 title=null 항목이 그 1건이다.
    experienceCount: 6,
    isBookmarked: true,
...
export const mockComprehensiveResult: ComprehensiveAnalysisResult = {
  ...
  isBookmarked: false,
  hasResultBody: true,
  // 분석에 넣은 경험 전부 — STAR 로 만들어진 것(career·club·award)과 근거가 모자라
  // 폐기된 것(certification·language)이 모두 여기 있어야 starAnalysisStatus 의 집계와 맞는다.
  experiences: [
    { id: "exp-demo-career", title: "자연어처리 연구실 학부 연구생" },
    { id: "exp-demo-club", title: "데이터 분석 학회 DataWave" },
    { id: "exp-demo-award", title: "전국 대학생 데이터 분석 공모전 우수상" },
    { id: "exp-demo-certification", title: "SQL 개발자 (SQLD)" },
    { id: "exp-demo-language", title: "영어 — 논문 독해와 실무 소통" },
  ...
  detailedSummary:
    "2023년 학회 정회원으로 시작해 스터디장, 학회장을 거치는 3년 동안 활동의 성격이 '참여'에서 '설계'로 옮겨간 흐름이 뚜렷합니다. 스터디를 운영하며 사람이 어디서 막히는지 관찰하는 습관이 생겼고, 공모전에서는 그 관찰을 데이터로 옮겨 심야 노선 후보 구간을 제안했으며, 연구실에서는 아예 데이터의 판단 기준 자체를 다시 만들었습니다. 세 축 모두 전후를 비교할 수 있는 정량 근거(이탈률 40%→12%, 187팀 중 2위, 일치도 0.61→0.78, 반복 실험 편차 ±0.004)를 갖췄고, 특히 의견이 갈렸을 때 논쟁 대신 두 기준을 각각 재보고 결과로 정한 사례는 신입 지원자에게서 보기 드문 서술입니다. 반면 분석 결과가 실제 의사결정으로 이어진 뒤의 이야기는 대부분 비어 있고, 사용한 통계 기법과 데이터 규모가 드러나지 않아 기술 면접에서 되물릴 여지가 남습니다. 어학·자격증은 점수와 보유 사실에서 서술이 끝나 다른 경험과 같은 수준의 근거를 갖추지 못했습니다.",
  keywordClustering: {
    personalityTendency: ["검증 지향", "책임감", "관찰형", "기준 설계형", "완주 지향"],
    coreCompetency: [
      "데이터 품질 관리",
      "공공데이터 분석",
      "실험 재현성",
    ...
    jobIndustry: ["데이터 분석", "데이터 사이언스", "AI 연구 지원", "공공·정책 데이터"],
  },
  experienceInsights: {
    motivation:
      "숫자가 맞는지 스스로 확인해야 마음이 놓이는 성향이 반복해 드러납니다. 결론을 내기 전에 기준부터 세우려는 태도가 네 건의 성과 모두에서 확인되고, 지원 동기 서술에서도 '데이터의 품질이 성능을 얼마나 좌우하는지 직접 확인하고 싶었다'처럼 검증 자체를 목적으로 둔 문장이 나옵니다. 남이 정해준 문제를 푸는 쪽보다 문제의 기준을 다시 놓는 쪽에 반복적으로 시간을 쓴 점이 일관됩니다.",
    learningPoints:
      "판단 기준을 문서로 만드는 법, 결과를 재현 가능하게 남기는 법, 같은 데이터를 다르게 보여주면 설득력이 달라진다는 점을 핵심 학습으로 정리할 수 있습니다. 여기에 더해 '이견은 논쟁이 아니라 측정으로 닫는다'는 원칙과, 사람이 이탈하는 지점을 먼저 찾고 나서 구조를 바꾸는 순서가 스터디 운영과 온보딩 개편에서 반복되며 하나의 방법론으로 굳어졌습니다.",
  },
  synergyCombinations: [
    {
    ...
    },
    {
      ...
      synergyReason:
        "사람이 어디서 막히는지 보고 커리큘럼을 고친 경험이, 심사위원을 설득한 발표 경험과 같은 근육을 씁니다. 어느 쪽도 '내용을 더 넣는' 방식이 아니라 받는 사람 기준으로 구조를 바꾼 방식이라는 점이 같습니다.",
      expectedEffect: "분석을 넘어 전달까지 가능한 사람으로 읽힘",
      applicableRoles: ["데이터 분석가", "비즈니스 애널리스트"],
    },
    {
      combinationTitle: "재현 가능한 실험 환경 + SQL 기반 검증",
      items: ["자연어처리 연구실 학부 연구생", "SQL 개발자 (SQLD)"],
      synergyReason:
        "설정을 분리하고 실행마다 지표를 자동 기록해 실험을 재현 가능하게 만든 경험과, 쿼리로 데이터를 직접 확인할 수 있는 자격이 겹치면 '결과를 남이 다시 만들 수 있게 남기는 사람'이라는 근거가 됩니다.",
      expectedEffect: "실험·지표 기반 조직에서 즉시 투입 가능한 인상",
  ...
  resumeStarFormat: [
    {
      ...
      situation:
        "레이블러 4명의 판단이 경계 사례마다 갈려 데이터셋의 신뢰도를 보장할 수 없었고, 특히 '인용된 혐오 표현'을 혐오로 볼지에서는 2주간 팀의 결론이 나지 않던 상황",
      task:
        "판단이 갈리는 경계 사례를 유형별로 정리해 같은 문장이면 같은 레이블이 붙게 만들고, 개선 여부를 같은 표본으로 다시 확인해 수치로 증명하는 것",
      action:
      ...
      result:
        "레이블러 간 일치도(Cohen's Kappa)가 0.61에서 0.78로 올랐고, 판단이 가장 많이 갈리던 풍자·인용 표현에서 불일치가 가장 크게 줄었습니다.",
      learning:
        "모델 구조를 바꾸는 것보다 판단 기준을 다시 세우는 편이 점수를 올렸다는 것. 이후로는 성능이 안 나올 때 모델보다 데이터를 먼저 들여다보는 순서가 습관으로 남았습니다.",
    ...
    {
      // 같은 경험(연구실)에서 나온 두 번째 성과 — 백엔드는 경험당 1건이 아니라 성과 단위로
      // STAR 를 만든다. 배움(L) 만 원문에 없어 삭제된 A 등급 사례다.
      title: "실험 파이프라인 정비로 반복 실험 F1 편차 ±0.004 확보",
      headline: "같은 설정이면 같은 결과가 나오도록 실험 환경을 정리해 리뷰 시간을 절반으로 줄임",
      situation:
        "같은 코드로 돌려도 실행할 때마다 결과가 달라 실험 간 비교가 성립하지 않았고, 원인조차 특정하지 못한 채 실험이 쌓이던 상황",
      task:
        "누가 언제 돌리든 설정이 같으면 같은 결과가 나오도록 실험 환경을 정리해, 실험 결과를 근거로 쓸 수 있게 만드는 것",
      action:
        "random seed를 고정하고 하이퍼파라미터를 JSON 설정 파일로 분리했으며, 실행마다 설정과 지표가 자동으로 기록되도록 Weights & Biases를 붙였습니다. 결과가 흔들리던 원인은 로그를 하나씩 대조해 데이터 로더의 셔플 시드가 따로 놀고 있었다는 것으로 특정했습니다.",
      result:
        "동일 설정 3회 반복 시 F1 편차가 ±0.004 이내로 줄었고, 멘토가 제 실험을 그대로 재현할 수 있게 되면서 리뷰 시간이 절반으로 줄었습니다.",
      learning: "",
      sourceQuotes: {
        situation: "초기에는 같은 코드인데도 결과가 매번 달라 원인을 찾지 못했습니다.",
        task: "같은 설정이면 누가 돌려도 같은 결과가 나오도록 실험 환경을 정리하는 것",
        action:
          "random seed를 고정하고 하이퍼파라미터를 JSON 설정 파일로 분리했습니다. 실행마다 설정과 지표가 자동으로 기록되도록 Weights & Biases를 붙였습니다.",
        result: "동일 설정 3회 반복 시 F1 편차가 ±0.004 이내로 줄었습니다.",
        learning: "",
      },
      competencyEvidence: [
        {
          competency: "실험 재현성",
          why: "설정을 코드에서 분리하고 자동 기록을 붙여 결과를 남이 다시 만들 수 있게 했습니다.",
        },
        {
          competency: "원인 규명",
          why: "재현되지 않는 현상을 감으로 넘기지 않고 로그 대조로 지점을 특정했습니다.",
        },
      ],
      evidenceStatus: {
        ...
        unsupportedSlots: [
          {
            slot: "L",
            label: "배움",
            reason: "이 성과에 대한 회고가 원문에 따로 적혀 있지 않습니다.",
            claimedQuote: "",
          },
        ],
        restructuringOnly: false,
      ...
      quality: {
        grade: "A",
        score: "9/10",
        verdict: "문제·조치·검증이 한 줄로 이어져요. 이력서에 그대로 옮겨도 좋아요.",
        criteria: [
          {
            key: "result_quantified",
            label: "결과 수치화",
            passed: true,
            detail: "F1 편차 ±0.004, 리뷰 시간 절반 수치 제시",
            coaching: "",
          },
          {
            key: "action_dominant",
            label: "Action 비중",
            passed: true,
            detail: "Action 이 전체의 42% (권장 40~50%)",
            coaching: "",
          },
          {
            key: "slots_distinct",
        ...
        priorityFixes: [],
        derivedFieldNotes: ["배움(L) 삭제: 이 성과에 대한 회고가 원문에 명시되지 않았습니다."],
      },
    },
    {
      ...
      situation:
        "3주차부터 난이도가 급격히 올라 스터디 이탈이 몰렸고, 이전 학기 중도 이탈률이 40%에 이르러 완주자보다 그만두는 사람이 더 눈에 띄던 상황",
      task:
  ...
  // 종합분석 백엔드는 아직 문자열을 보낸다 — 매퍼가 content 에 담고 기간·마감일은 빈다.
  actionPlan: {
    shortTerm: {
      period: "",
      deadline: "",
      content:
        "각 경험의 시작점(어떤 문제였는지)을 한 문장씩 앞에 붙이고, 공모전 분석에 쓴 데이터의 기간과 건수를 명시하세요. 어학·자격증 두 항목에는 그 능력으로 실제 한 일을 한 줄씩 붙여 STAR 로 만들 수 있는 상태로 바꾸는 것이 3개월 안에 가장 효율이 좋은 작업입니다.",
    },
    midTerm: {
      period: "",
      deadline: "",
      content:
        "분석 결과가 실제 결정으로 이어진 사례를 1건 확보하세요. 개정한 가이드라인이 데이터셋 v2 구축에 어떻게 쓰였는지 확인해 기록에 남기거나, 학회 발표회에서 제안이 채택된 사례를 만들면 지금 가장 큰 공백인 '분석 이후'가 채워집니다. 같은 기간에 사용한 통계 기법과 선택 이유를 각 경험에 한 줄씩 덧붙이세요.",
    },
    longTerm: {
      period: "",
      deadline: "",
      content:
        "공공데이터 도메인에서 분석-제안-검증을 완결한 프로젝트를 하나 만드세요. 제안에서 끝내지 말고 적용 후 지표 변화까지 측정하는 것이 목표입니다. 과정을 공개 글로 남기면 지금 부족한 교외 노출까지 함께 해결됩니다.",
    },
  },
  strengthDiagnosis: {
    oneLineVerdict:
    ...
    strengths: [
      ...
      },
      {
        ...
        title: "3년간 같은 방향의 성장",
        diagnosis:
          "2023년 정회원으로 시작해 2024년 스터디장, 2025년 학회장으로 역할이 넓어졌고, 그 사이 연구실·공모전 경험이 같은 데이터 축 위에 쌓였습니다. 활동을 옮겨 다닌 흔적이 아니라 한 방향으로 깊어진 흐름입니다.",
        evidence: "역할 이력 3단계(정회원 2023 → 스터디장 2024 → 학회장 2025)가 기간과 함께 기록됨",
        impact: "'오래 남아 조직을 바꾼 사람'으로 읽혀 장기 근속 신뢰를 얻기 유리",
        leverageAction: "이력서 첫 줄에 역할 변화를 기간과 함께 한 줄로 적으세요.",
      },
      {
        id: "s-5",
        category: "활동_수량",
        ...
        title: "직무 축에 모인 활동 수",
        diagnosis:
          "연구실 인턴 1회, 수상 1회, 학회 3년, 교육과정 1회가 모두 데이터 축에 모여 있습니다. 개수 자체보다 흩어지지 않았다는 점이 이 조합의 강점입니다.",
        evidence: "분석 대상 6건 중 5건이 데이터·분석 직무와 직접 연결됨",
        impact: "서류에서 '무엇을 하고 싶은 사람인지' 되묻게 만들지 않음",
        leverageAction: "직무와 무관한 활동은 이력서에서 과감히 빼고 이 축만 남기세요.",
      },
    ],
    noStrengthDiagnosis: {
      hasIssue: false,
    ...
    standoutExperienceTypes: ["인턴 및 업무 경력", "수상 경력", "동아리 · 학회"],
    contentQualityHighlights: [
      {
        item: "'실험 파이프라인 정비' 항목",
        highlight: "재현되지 않는 원인을 로그 대조로 특정한 과정과 편차 ±0.004라는 결과가 함께 있음",
        whyEffective: "'환경을 정리했다'는 흔한 서술과 달리 무엇이 얼마나 안정됐는지 확인 가능함",
      },
      {
        item: "'2025 신입 온보딩 개편' 항목",
        highlight: "설문 62건으로 원인을 확인한 뒤 개편했고 지원자 24명→41명까지 이어짐",
        whyEffective: "의견 대신 데이터로 방향을 정한 순서가 그대로 드러남",
      },
      {
        item: "'레이블링 기준 개정' 항목",
  ...
  criticalDiagnosis: {
    ...
    weaknesses: [
      ...
      },
      {
        ...
        title: "교외 노출이 적음",
        diagnosis:
          "활동 대부분이 교내 연구실과 교내 학회에 몰려 있습니다. 외부 커뮤니티 발표나 공개 글쓰기처럼 교외에서 검증된 흔적은 공모전 1건뿐입니다.",
        evidence: "분석 대상 6건 중 교외 활동은 공모전과 교육과정 2건",
        impact: "비슷한 지원자와 겹치는 영역에서 차별점을 만들기 어려움",
        priorityAction: "가이드라인 개정 과정을 공개 글 한 편으로 정리해 올리세요.",
      },
    ],
    missingExperienceTypes: ["기업 데이터 직무 인턴", "외부 발표·기고", "실서비스 데이터 경험"],
    contentQualityIssues: [
      ...
      },
      {
        ...
        issue: "취득 사실과 활용 계획만 있고 실제로 쿼리를 써서 해결한 사례가 없음",
        improvementHint: "스터디 실습 문제를 직접 만든 일을 이 자격의 활용 사례로 옮겨 적으세요",
      },
    ],
    competitorGap:
      "비슷한 지원자 대비 분석 과정의 신뢰도는 앞서지만, 분석 결과의 활용까지 이어진 사례 수는 평균 수준입니다. 같은 학교·학번의 데이터 직무 지원자가 보통 갖추는 기업 인턴 경험과 실서비스 규모의 데이터를 다뤄본 이력이 없고, 다룬 데이터가 모두 공개 데이터셋과 공공데이터에 머물러 있습니다.",
  },
  verifiedJobs: [
    {
      company: "토스",
      ...
      deadline: "2026-12-30",
      whyMatch:
        "공고가 요구하는 'SQL 로 직접 데이터를 확인하고 가설을 검증하는 역량'이 공모전의 승하차·상권 데이터 결합 분석과 SQLD 보유로 직접 연결됩니다. 채용 페이지가 강조하는 '지표를 의심하는 태도'도 본선 사흘 전 기준 시간대 오류를 스스로 발견해 결과를 다시 뽑은 사례로 답할 수 있습니다.",
    ...
    },
    {
      ...
      deadline: "상시채용",
      whyMatch:
        "A/B 테스트를 상시 운영하는 조직이라 '같은 설정이면 같은 결과가 나오는 실험 환경'을 직접 만들어 본 경험이 그대로 쓰입니다. seed 고정·설정 분리·자동 기록으로 반복 실험 편차를 ±0.004까지 줄인 사례는 신입 지원자에게서 거의 나오지 않는 근거입니다.",
    ...
    },
    {
      ...
      deadline: "2026-09-15",
      whyMatch:
        "이미 이 기관이 주최한 공모전에서 우수상을 받아 도메인과 데이터 형식에 익숙하고, 공공데이터를 정책 관점으로 해석한 경험이 직무 설명과 겹칩니다. 지금 가장 큰 공백인 '분석 이후의 활용'을 채우기에도 적합한 자리입니다.",
      url: "https://example.com/jobs/kdata-intern",
      isValid: true,
    },
  ],
  expiredJobs: [
